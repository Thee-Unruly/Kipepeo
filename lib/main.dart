...
class _DashboardPageState extends State<DashboardPage> {
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Text(
                hasData
                    ? "Tip: Recording stock purchases builds trust."
                    : "Tip: Tap 'Update Profile' to begin.",
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 11,
                  color: color,
  ...
  void _showVisualReport(BuildContext context) {
    ...
    if (_loanHistory.isNotEmpty) {
      avgUtilization =
          _loanHistory.fold(0.0, (sum, l) => sum + l.businessUtilization) /
          _loanHistory.length;
      onTimeRepayments = _loanHistory
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: _buildStatTile(
                          'CASH FLOW',
                          currency.format(_currentProfile!.avgMonthlyInflow),
                          Icons.trending_up,
                        ),
                      ),
                      const SizedBox(width: 16),
                  ...
                  _buildStatTile(
                    'BUSINESS USAGE',
                    '${(avgUtilization * 100).toStringAsFixed(0)}% of funds used for stock',
                    Icons.shopping_cart,
                  ),
                  const SizedBox(height: 24),
                  _buildReportSection(
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        ...(_governanceResult!.warnings.isEmpty
                            ? [
                                const Text(
                                  '• No risky debt patterns detected',
                                  style: TextStyle(fontSize: 13),
                                ),
                              ]
                            : _governanceResult!.warnings.map(
                                (w) => Text(
                                  '• $w',
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'This borrower uses ${(avgUtilization * 100).toStringAsFixed(0)}% of their loans directly on business stock.',
                          textAlign: TextAlign.center,
                          style: const TextStyle(
                            fontSize: 13,
...
class _AuditHistoryPageState extends State<AuditHistoryPage> {
  ...
  final String _pId =
      '96324880c551793f7739545464197e411c5210c14f09a5601a457494f6f89069';
  ...
  Widget _buildTransactionLedger() {
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (allTxs.isEmpty)
          return const Center(
            child: Text('No business payments verified yet.'),
          );
        return ListView.builder(
          padding: const EdgeInsets.all(16),
          ...
          itemBuilder: (context, i) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  '${isCash ? "Cash" : "Mobile"} • ${DateFormat('dd MMM').format(tx.timestamp)}',
                ),
                trailing: IconButton(
                  icon: const Icon(
                    Icons.delete_sweep,
                  ...
                  onPressed: () async {
                    if (isCash)
                      await _db.deleteCashTransaction(tx.id);
                    else
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatRow(
              'Used for Business',
              '${(loan.businessUtilization * 100).toStringAsFixed(0)}%',
            ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildToggleTile(
            'Privacy Shield',
            'Active (Your data is hidden)',
            true,
          ),
          _buildToggleTile(
            'On-Phone Math',
            'Reports are built right here',
            true,
          ),
          _buildToggleTile(
            'On-Phone Memory',
            'Raw records never leave phone',
            true,
          ),
