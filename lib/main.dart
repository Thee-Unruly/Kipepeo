...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_currentUser == null) return LandingPage(onLogin: (u) => setState(() => _currentUser = u));
    return MainNavigation(user: _currentUser!, onLogout: () => setState(() => _currentUser = null));
  }
}
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Empowering Mama Mbogas Everywhere', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey)),
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _loadData() async {
    ...
    if (txs.isNotEmpty || cashTxs.isNotEmpty) {
      final profile = _featureService.generateProfile(widget.user.phoneNumber, txs);
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 32),
            Text(hasData ? "Your business is being tracked successfully." : "Welcome, ${widget.user.fullName}! Sync data to begin.", textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w500, color: Colors.black87, fontSize: 13)),
          ],
        ),
      ),
    );
  ...
  Widget _buildReportSection(String title, Widget content) {
    return Container(width: double.infinity, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.grey.shade200)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    return FutureBuilder(future: Future.wait([_db.getTransactions(), _db.getCashTransactions()]), builder: (context, snapshot) {
      ...
      return ListView.builder(padding: const EdgeInsets.all(16), itemCount: allTxs.length, itemBuilder: (context, i) {
        ...
        return Card(margin: const EdgeInsets.only(bottom: 8), child: ListTile(
          leading: Icon(tx.type == TransactionType.outflow ? Icons.shopping_bag : Icons.payments, color: Colors.teal),
