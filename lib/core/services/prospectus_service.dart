...
class ProspectusService {
  ...
  String generateProspectus(
    ...
    final String healthStatus = profile.riskScore > 0.7
        ? "VERY STRONG"
        : profile.riskScore > 0.4
        ? "STEADY"
        : "GROWING";
    ...
    if (loanHistory.isNotEmpty) {
      avgUtilization =
          loanHistory.fold(0.0, (sum, l) => sum + l.businessUtilization) /
          loanHistory.length;
      onTimeRepayments = loanHistory
...
''';
  ...
  String generateDailyPassport(
    ...
    double totalInflow = dailySnapshots.fold(
      0,
      (sum, s) => sum + s.totalInflow,
    );
    ...
    double avgSafeToSpend =
        dailySnapshots.fold(0.0, (sum, s) => sum + s.safeToSpend) /
